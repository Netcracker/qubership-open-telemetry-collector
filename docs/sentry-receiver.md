...
| `exception.values` or flat `exception[]`                                         | `stacktrace` or `full_message` | Sentry exceptions are normalized to the `exception.values` span attribute.            | [Exception Interface](https://develop.sentry.dev/sdk/telemetry/errors/#exception-interface) |
...
- sentry_session_exited_count - allows to monitor the number of unique sessions with `status: "exited"`
