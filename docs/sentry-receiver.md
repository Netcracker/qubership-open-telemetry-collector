...
Usually it has a structure like:

```json
{ ... } // header - contains information on who and how to send that event.
...
- **`session`** - envelope of this type means that the user started a `session` on the page.
  This event just indicates that a User-Agent opened the page.
- **`transaction`** - this envelope contains `spans`, `breadcrumbs`, `measurements`, and much other information.
  Usually, a transaction has a start time and an end time.
...
In the table below, you can find the mapping for the fields of Sentry envelopes of types **event** and **transaction**
...
In the table below, you can find a mapping of Sentry **spans** fields to the attributes of opentelemetry spans:
...
LogTCP Exporter allows logging certain data from Sentry envelopes to Graylog.
...
| `exception.values` or flat `exception[]`                                         | `stacktrace` or `full_message` | Sentry exceptions are normalized to the `exception.values` span attribute.             | [https://develop.sentry.dev/sdk/telemetry/errors/#exception-interface](https://develop.sentry.dev/sdk/telemetry/errors/#exception-interface) |
...
| `logger`                                                                         | `category`                     | if logger present, use it in the category field. Otherwise use `frontend-event` value | _new field_                                                                                                                                  |
...
Envelopes with the type `session` are not logged to the logging system.

## Sentry Envelope to Metrics

SentryMetrics Connector allows generating metrics for each type of Sentry envelopes below:

### `type: "session"` (Metrics)

- sentry_session_exited_count - allows to monitor the number of unique sessions with `status: "exited"` when the session ends.
...
- sentry_event_count - allows to monitor the number of Sentry events by `event.level`
